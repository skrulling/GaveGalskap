...
import { AskGpt } from "~/openai.server";
import { SafeGift } from "~/types/gift";

export let action: ActionFunction = async ({ request }) => {
  const formData = await request.formData();
  const gifts = formData.get("gifts");
  const parsed = JSON.parse(gifts as string) as SafeGift[];

  const response = await AskGpt(parsed); // Call your server function here.
...
const Suggestions: React.FC<SuggestionsProps> = ({ gifts }) => {
